...
DEFAULT_REGULONDB_ENDPOINT = "https://regulondb.ccg.unam.mx/graphql"
...
def download_assets(manifest: dict[str, Any], root: str | Path | None = None, lock_path: str | Path | None = None, refresh_lock: bool = False) -> dict[str, Any]:
    ...
    lock_file = Path(lock_path) if lock_path else root_path / "config" / "regulondb.lock.json"
...
def validate_manifest(manifest: dict[str, Any], root: str | Path | None = None, check_files: bool = True, lock: dict[str, Any] | None = None, require_lock: bool = False) -> list[str]:
    ...
    if precise_assets and (model is None or expression is None):
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--manifest", default=root / "config" / "regulondb_assets.json")
    parser.add_argument("--root", default=root)
    parser.add_argument("--lock", default=root / "config" / "regulondb.lock.json")
